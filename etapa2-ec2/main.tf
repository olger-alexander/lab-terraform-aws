...
locals {
  mi_ip_cidr = "${trimspace(data.http.mi_ip.response_body)}/32"
}

# ---------- Data sources: consultamos lo que ya existe ----------
...
# ---------- IAM: rol para administrar la instancia SIN SSH ----------
...
resource "aws_security_group" "web" {
  ...
  tags        = { Name = "${local.prefijo}-sg-web" }
}

resource "aws_vpc_security_group_ingress_rule" "http" {
  security_group_id = aws_security_group.web.id
  description       = "HTTP solo desde mi IP"
  cidr_ipv4         = local.mi_ip_cidr
...
resource "aws_instance" "web" {
  ...
  user_data = <<-EOF2
    ...
    echo "<h1>Etapa 2 - Servidor ${count.index} de ${var.alumno} desplegado con Terraform</h1>" \
  ...
  tags = { Name = "${local.prefijo}-web-${count.index}" }
}
